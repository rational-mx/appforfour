...
(function() {
  var ConvertAllLinks, IsLinK, number_of_comments;

  number_of_comments = 5;

  $(document).on("ready page:change", function() {
    return $("#submit_comment").on("click", function() {
      return ConvertAllLinks();
    });
  });

  ConvertAllLinks = function() {
    var comment, i, j, ref, words;
    comment = $('#comment_body').val();
    words = comment.split(' ');
    for (i = j = 0, ref = words.length; j <= ref; i = j += 1) {
      words[i] = IsLinK(words[i]);
    }
    words = words.join(' ');
    return $('#comment_body').val(words);
  };
...
(function() {
  var AnalizePostBody, InsertLink, InsertYoutube, already, flip_flop;
  ...
  $(document).on("ready page:change", function() {
    var browserName, nAgt, nVer, osName;
    $("#post_body").keyup(function() {
      if (!already) {
        return AnalizePostBody($("#post_body").val());
      }
    });
    ...
    return $(window).scroll(function() {
      if (flip_flop) {
        if ($(window).scrollTop() + $(window).height() === $(document).height()) {
          $('#more_comments').click();
          return flip_flop = false;
        }
      } else {
        return flip_flop = true;
      }
    });
  });

  AnalizePostBody = function(body) {
    var youtube_key;
  ...
  InsertLink = function(container, url_link) {
    var url;
    url = body.match(/^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/);
    return $("#post_media_url").val(url[0]);
  };
  ...
  InsertLink = function(container, url_link) {
    ...
    return container.html("<a href='" + url_link + "''>" + url_link + "</a>");
  };

  InsertYoutube = function(container, youtube_key) {
    $("#post_media_url").val("<embed width= '320' height='190' src = 'http://www.youtube.com/v/" + youtube_key + "'>");
    return container.html("<embed width= '320' height='190' src = 'http://www.youtube.com/v/" + youtube_key + "'>");
  };
